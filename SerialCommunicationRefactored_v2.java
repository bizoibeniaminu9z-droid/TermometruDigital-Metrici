...
/**
 * Serial helper with minimized branching:
 *  - data is accumulated and split by any line break (\\R) in one pass
 *  - no per-char scanning; very few conditionals
...
public final class SerialCommunication {
    ...
    private void readAndEmitLines() {
        ...
        String[] parts = str.split("\\R", -1); // keep trailing empty
        int complete = parts.length - 1;
        for (int i = 0; i < complete; i++) emit(parts[i].trim());
        buffer.setLength(0);
        buffer.append(parts[parts.length - 1]); // tail (possibly empty)
    }
